...
public class SpecialCheckOut implements CheckOut {
	...
	public void checkOutBook(Member loginMember) {
		...
		if (loginMember.getCheckOutBookList().size() >= Define.SPECIAL_LIMIT) {
			System.out.println("도서 대여 한도 권수를 초과했습니다.");
			System.out.println("특별 회원은 동시에 5권까지만 대여가능합니다.");
		} else {
			...
			book.setCheckOutPeriod(Define.SPECIAL_PERIOD);
	...
	public void returnBook(Member loginMember) {
		...
		book.setCheckOutPeriod(Define.GENERAL_PERIOD);
